  ...
  return (
    <div className="group flex flex-col gap-1 rounded-lg border border-border/70 bg-elevated px-2.5 py-2">
      <div className="flex items-center gap-1.5">
        ...
        <span className="shrink-0 text-[10px] text-subtle">{timeAgo(item.ts)}</span>
        <button
      ...
      >
        Jump to terminal <ArrowRight size={12} />
      </button>
    </div>
  );
}
...
export function NotificationBell() {
  ...
  useEffect(() => {
    const t = window.setInterval(() => {
      const now = Date.now();
      const { items: cur, push: _p, ..._rest } = usePermStore.getState();
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute bottom-full right-0 z-50 mb-2 flex max-h-[70vh] w-80 flex-col overflow-hidden rounded-xl border border-border bg-surface shadow-xl">
          <div className="flex h-9 shrink-0 items-center justify-between border-b border-border px-3">
            <span className="text-[12px] font-semibold text-fg">Approval requests</span>
            {hasItems && (
              <button
                onClick={clear}
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {hasItems ? (
              ...
              <p className="px-2 py-6 text-center text-[11px] text-subtle">
                No pending approvals. We'll notify you here (and with a system
                notification) when a coding agent waits for your OK.
              </p>
            )}
          </div>
        </div>
